...
async def create_keyboards_for_symbol_for_fundamentals(message, res, string):
    keyboard = InlineKeyboardBuilder()
    fundaments_dict[f'{message.chat.id}'] = []
    try:
        for name_attr in res.json()['symbols']:
            ...
            fundaments_dict[f'{message.chat.id}'].append(
                name_attr['name'].replace('</b>', '').replace('<b>', '') + f'{string}')
        keyboard.adjust(2)
        keyboard.row(
            InlineKeyboardButton(text='Назад (выбрать другой тикер)', callback_data=('back_to_symbol' + 'fundamentals')))
